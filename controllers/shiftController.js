...
async function createShift(req, res) {
    try {
        ...
        const newShift = new shift({ code: code.toUpperCase(), name, startTime, endTime, isNight: !!isNight});
        await newShift.save();
        res.status(201).json({success: true, data: newShift, message: 'tạo mã ca thành công'});
    }catch (err) {
        if (err.code === 11000){
            return res.status(400).json({success: false, message: 'Mã ca đã tồn tại'});
        }
        console.error(err);
